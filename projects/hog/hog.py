...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        temp = dice()
        if temp == 1:  # mark pigout but not stop the while loop
            pig_out = True
        sum += temp
        num_rolls -= 1
    if pig_out:
    ...
    def say(score0, score1, last_score=last_score, running_high=running_high):
        if who == 0:
            if score0 - last_score > running_high:
                ...
                # running_high changes whenever "bigger", otherwise stays the same
                running_high = score0 - last_score
            last_score = score0  # last_score changes as long as it's who's turn
        if who == 1:
            ...
            last_score = score1
        # return to update the varaiable
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # original_function = make_test dice
    # *args = (4,2,5,1)
    # thus run original_function(*args) 1000 times then get the average
    # new_func shares the same arguments of original functions
    # new_func(*args) = what the function looking for (whatever make average, make floor,etc)
    def new_func(*args):
        total = 0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    # Find max_scoring_number_rolls => the biggst make average()
    # Make_Averaged with roll_dice, 1000
    # make_averaged(roll_dice, trials_count=1000)
    # use while loop
    # use a higher order function to call make_averaged
    # use a higher function to call which average_roll with how manytimes rolled
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score+free_bacon(opponent_score), opponent_score):
        return 0
    elif not extra_turn(score+free_bacon(opponent_score), opponent_score) and free_bacon(opponent_score) >= cutoff:
        return 0
    else:
        return num_rolls
